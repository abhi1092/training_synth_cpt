...
def _get_quality_graph(model_name: str) -> List[str]:
    files = _glob_all_json(f'data/quality_entigraph_{model_name}-pruned-v1.1')
    result = []
    for file in files:
        content = jload(file)
...
def tokenize_quality_graph(model_name: str):
    ...
    write_to_memmap_single(tokenize_list(quality), f'quality_all-entigraph{model_name}-pruned-v1.1.bin')
